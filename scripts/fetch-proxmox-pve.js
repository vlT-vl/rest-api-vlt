...
const SEARCH_URL = 'https://git.proxmox.com/?p=pve-manager.git;a=search;s=bump+version+to;st=commit';
const OUTPUT = path.resolve(__dirname, '../v1/proxmox/pve/releases.json');
...
function parseCommitRows(html) {
  const results = [];
  // Match table rows; gitweb uses class="light" and class="dark" alternation
  const rowRe = /<tr[^>]*class="(?:light|dark)"[^>]*>([\s\S]*?)<\/tr>/gi;
  let rowMatch;
  while ((rowMatch = rowRe.exec(html)) !== null) {
    const rowHtml = rowMatch[1];
    // Extract commit message / subject
    ...
    // Extract date — gitweb shows either ISO date in <i> or in age span title attribute
...
async function main() {
  ...
  for (const { version, date } of commits) {
    ...
    newEntries.push({
      'Nome Release': `Proxmox VE ${version}`,
      'Versione': version,
      'Codename': `pve-${version}`,
  ...
  if (!newEntries.length) {
    console.log('proxmox-pve: no new releases');
    return;
  }
  ...
  console.log(`proxmox-pve: added ${newEntries.map(r => r['Versione']).join(', ')}`);
}

main().catch(err => { console.error('proxmox-pve:', err.message); process.exit(1); });
