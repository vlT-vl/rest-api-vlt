...
const SEARCH_URL = 'https://git.proxmox.com/?p=proxmox-datacenter-manager.git;a=search;s=bump+version+to;st=commit';
const OUTPUT = path.resolve(__dirname, '../v1/proxmox/pdm/releases.json');
...
async function main() {
  ...
  for (const { version, date } of commits) {
    ...
    newEntries.push({
      'Nome Release': `Proxmox Datacenter Manager ${version}`,
      'Versione': version,
      'Codename': `pdm-${version}`,
  ...
  if (!newEntries.length) {
    console.log('proxmox-pdm: no new releases');
    return;
  }
  ...
  console.log(`proxmox-pdm: added ${newEntries.map(r => r['Versione']).join(', ')}`);
}

main().catch(err => { console.error('proxmox-pdm:', err.message); process.exit(1); });
